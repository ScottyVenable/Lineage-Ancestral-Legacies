...
namespace Lineage.Tools
{
    ...
    public class EntityMigrationTool
    {
        ...
        public static void MigrateAllPopsInScene()
        {
            ...
            Debug.Log($"Successfully migrated {convertedEntities.Count} Pops to Entity system");
        }
    ...
    public static Entities.Entity MigratePop(Pop pop)
        {
            if (pop == null)
            {
                Debug.LogError("Cannot migrate null Pop object");
                return null;
            }
            ...
            Debug.Log($"Migrating Pop '{originalName}' to Entity system...");
            ...
            try
            {
                ...
                Debug.Log($"✓ Successfully migrated '{originalName}' to Entity system");
                return entity;
            }
            catch (System.Exception e)
            {
                Debug.LogError($"Failed to migrate Pop '{originalName}': {e.Message}");
                return null;
            }
        }
        ...
        private static void ApplyMigrationData(Entities.Entity entity, MigrationData data)
        {
            ...
            Debug.Log($"Applied migration data for entity '{data.name}'");
        }
    ...
    public class EntityMigrationWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Find Pops in Scene"))
            {
                ...
                Debug.Log($"Found {pops.Length} Pop objects in scene:");
                foreach (Pop pop in pops)
                {
                    Debug.Log($"- {pop.popName} at {pop.transform.position}");
                }
            }
            ...
            if (GUILayout.Button("Validate Entities in Scene"))
            {
                ...
                foreach (Entities.Entity entity in entities)
                {
                    ...
                    {
                        Debug.LogWarning($"Entity '{entity.EntityName}' failed validation", entity);
                    }
                }
                Debug.Log($"Validated {validCount}/{entities.Length} entities successfully");
            }
        }
    }
#endif
