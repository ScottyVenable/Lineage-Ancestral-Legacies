...
namespace Lineage.Ancestral.Legacies.Database
{
    ...
    public enum RarityType
    {
        ...
        Buff, // For buffs that can be applied to entities        Quest // For quests that can be undertaken
    }
    ...
    public struct StatModifiers
    {
        ...
        {
            stat = default; // Explicitly assign default value to stat
            flatModifier = flat;
    ...
    public struct Entity
    {
        ...
        public void ChangeState(Database.State.ID newStateID)
        {
    ...
    {
        /// <summary>
        /// Unique identifiers for predefined settlement types.
        /// </summary>
        public enum ID
        {
            Village = 0,
        ...
        /// <summary>
        /// The unique identifier for this settlement, often corresponds to <see cref="ID"/>.
        /// </summary>
        public int settlementID;
        /// <summary>The display name of the settlement.</summary>
        public string settlementName;
        /// <summary>A short description of the settlement.</summary>
        public string settlementDescription;
        /// <summary>The icon representing this settlement in UI elements.</summary>
        public Sprite settlementIcon;
        public bool isPlayerControlled;

        // Settlement Properties
        /// <summary>The type of this settlement (e.g., Village, City).</summary>
        public ID settlementType;
        /// <summary>The size category of this settlement (e.g., Small, Medium).</summary>
        public string settlementSize;
        /// <summary>The faction that controls this settlement.</summary>
        public string controllingFaction;

        // Population and Resources
        /// <summary>The current population of the settlement.</summary>
        public int population;
        /// <summary>The maximum population capacity of the settlement.</summary>
        public int maxPopulation;
        /// <summary>The resources available in the settlement (e.g., Gold, Wood).</summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Settlement"/> struct.
        /// </summary>
        /// <param name="name">The name of the settlement.</param>
        /// <param name="id">The unique ID of the settlement.</param>
        /// <param name="type">The type of the settlement.</param>
        /// <param name="size">The size category of the settlement.</param>
        /// <param name="faction">The controlling faction.</param>
        /// <param name="description">A short description of the settlement.</param>
        /// <param name="icon">The icon representing this settlement in UI elements.</param>
        /// <param name="population">The current population of the settlement.</param>
        /// <param name="maxPopulation">The maximum population capacity of the settlement.</param>
        public Settlement(string name, int id, ID type, string size, string faction, string description, Sprite icon, int population, int maxPopulation)
        {
            settlementName = name;
            settlementID = id;
            settlementType = type;
            ...
            settlementIcon = icon;
            isPlayerControlled = false; // ensure this field is assigned
    ...
    public struct Item
    {
        ...
        public struct Buff
        {
            ...
            public StatModifiers statModifiers; // Optional: Use for buffs that modify stats directly
        ...
        public struct NPC
        {
            ...
            public enum ID
            {
                // Generate entities for the following people: Akka, Kitoma, Zejo, Kiba
                Akka = 0, // Example NPC ID for a trader named Akka
                Kitoma = 1, // Example NPC ID for a warrior named Kitoma
                Zejo = 2, // Example NPC ID for a scholar named Zejo
                Kiba = 3, // Example NPC ID for a healer named Kiba
            }
        ...
        public class TimelineEvent
        {
            ...
            public TimelineEvent(string title, string date, string description, EventImportance importance = EventImportance.Minor)
            {
                Title = title;
                Date = date;
        ...
        public class LoreEntry
        {
            ...
            public string Title { get; set; } = "";
            public string Category { get; set; } = "History"; // Changed from enum to string for flexibility
            ...
            }            public LegacyCategory category
            {
                get 
                {
                    return Category?.ToLower() switch
                    {
                        ...
                        "legend" => LegacyCategory.Legend,
                        "technology" => LegacyCategory.Technology,
                        "culture" => LegacyCategory.Culture,
                        "geography" => LegacyCategory.Geography,
                        "bestiary" => LegacyCategory.Bestiary,
                        _ => LegacyCategory.History
                    };
                }
                set 
                {
            ...
            public LoreEntry(string title, LegacyCategory category, string content)
            {
                Title = title;
            ...
            public LoreEntry(string title, string category, string content)
            {
                Title = title;
                Category = category;
                Content = content;
            ...
            /// <summary>
            /// Creates a deep copy of this LoreEntry.
            /// </summary>
            /// <returns>A new LoreEntry instance with copied values.</returns>
            public LoreEntry Clone()
            {
                return new LoreEntry
                {
                    ...
                    Source = this.Source,
                    RelatedCharacters = new List<string>(this.RelatedCharacters),
                    RelatedLocations = new List<string>(this.RelatedLocations),
                    RelatedEvents = new List<string>(this.RelatedEvents),
                    CrossReferences = new List<string>(this.CrossReferences),
                    TimelinePosition = this.TimelinePosition,
                    Duration = this.Duration,
                    TimelineEvents = this.TimelineEvents.Select(te => new TimelineEvent(te.Title, te.Date, te.Description, te.Importance)).ToList(),
                    Era = this.Era,
                    Age = this.Age,
        ...
        public class Population
        {
            public enum ID
            {
                Kaari = 0,     // Represents a generic population entity
                Lineage = 1,   // Represents a lineage of entities
                Tribe = 2,     // Represents a tribe of entities
                Clan = 3,      // Represents a clan of entities
                Faction = 4    // Represents a faction of entities
            }
            ...
            /// <summary>
            /// Adds an entity to this population if not already present.
            /// </summary>
            public void AddMember(Entity entity)
            {
                if (_members.Exists(e => e.entityID == entity.entityID) == false)
                    _members.Add(entity);
            }

            /// <summary>
            /// Removes an entity by its ID. Returns true if removed.
            /// </summary>
            public bool RemoveMember(int entityID)
            {
                var existing = _members.Find(e => e.entityID == entityID);
            ...
            /// <summary>
            /// Clears all members from this population.
            /// </summary>
            public void Clear()
            {
                _members.Clear();
            ...
            /// <summary>
            /// Generates a batch of initial members using the provided factory function.
            /// </summary>
            public void GenerateInitial(int count, Func<Entity> factory)
            {
                for (int i = 0; i < count; i++)
                    _members.Add(factory());
            }

            /// <summary>
            /// Returns how many members have the specified EntityType.
            /// </summary>
            public int CountByType(Entity.EntityType type)
            {
                return _members.Count(e => e.entityType.Contains(type));
            }

            /// <summary>
            /// Returns a random member, or null if empty.
            /// </summary>
            public Entity? GetRandomMember()
            {
                if (_members.Count == 0) return null;
                int idx = UnityEngine.Random.Range(0, _members.Count);
                return _members[idx];
            }
        }
